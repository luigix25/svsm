...
impl VsockTransport for VirtIOVsockDriver {
    fn connect(&self, remote_cid: u32, local_port: u32, remote_port: u32) -> Result<(), SvsmError> {
        ...
        loop {
            ...
            dev.wait_for_event()?;
            let status = dev.is_connection_established(server_address, local_port)?;
...
impl From<virtio_drivers::Error> for SvsmError {
    fn from(e: virtio_drivers::Error) -> Self {
        ...
        match e {
            SocketDeviceError(SocketError::ConnectionExists) => {
                Self::Vsock(VsockError::ConnectionExists)
            }
            SocketDeviceError(SocketError::NotConnected) => Self::Vsock(VsockError::NotConnected),
            SocketDeviceError(SocketError::PeerSocketShutdown) => {
                Self::Vsock(VsockError::PeerSocketShutdown)
            }
            _ => Self::Vsock(VsockError::DriverError),
        }
    }
}
